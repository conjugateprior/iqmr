...
subcorp <- corpus_subset(corpus_uk_manif, year %in% c(2017, 2015, 2010, 2005) & party %in% c('Con', 'Lab', 'LD', 'UKIP', 'SNP'))
dfmsubcorp <- dfm(subcorp, remove_punct=TRUE)
con2017 <- docvars(dfmsubcorp, "party") == "Con" & docvars(dfmsubcorp, "year") == 2017
con2017dfm <- dfmsubcorp[con2017,]
used <- sum((colSums(con2017dfm)) != 0) # used at least once
used_per <- used/ncol(con2017dfm)
hapaxes <- sum((colSums(con2017dfm)) == 1)
hapaxes_per <- hapaxes/ncol(con2017dfm)
@

Example: Conservative party 2017 manifesto compared to other parties in two elections\\
~\\
\begin{tabular}{ll}
High D. & \Sexpr{ncol(dfmsubcorp)} word types\\
          & (adult native english speakers know $\sim$20-35,000) \\
Sparse & Of these, Con 2017 only uses \Sexpr{used}
  (\Sexpr{round(used_per*100, 2)}\%)\\
Skewed &  Of these \Sexpr{hapaxes}
  (\Sexpr{round(100*hapaxes_per, 2)}\%) words appear exactly once
\end{tabular}
...
More generally: Zipf-Mandelbrot law (a Pareto distribution in disguise)
\begin{align*}
...
tf <- topfeatures(dfmsubcorp)
tbl <- data.frame(Count=tf, row.names=names(tf))
...
tf <- topfeatures(dfmsubcorp, decreasing=FALSE)
tbl <- data.frame(Count=tf, row.names=names(tf))
print(autoformat(xtable(tbl)), booktabs=TRUE)
@
\end{center}
...
tf <- topfeatures(dfm_remove(dfmsubcorp, features=stopwords("english")))
tbl <- data.frame(Count=tf, row.names=names(tf))
print(autoformat(xtable(tbl)), booktabs=TRUE)
@
\end{center}
...
\itm Words you'd want to keep when analyzing an abortion debates.
\itz
\end{frame}
...
\end{frame}
\begin{frame}[t,fragile]\frametitle{Inference About proportions: DROP THIS}

<<>>=
data(dict_lavergarry)
dfmsubcorp_lg <- dfm_lookup(dfmsubcorp, dict_lavergarry) # gives aother dfm
con2017dfm_lg <- dfmsubcorp_lg[con2017,]
@
...
\end{frame}
\begin{frame}[t]\frametitle{Inference about proportions: DROP THIS}
