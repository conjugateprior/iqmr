...
corpdfm <- dfm(corp)
```
and run a simple one dimensional scaling model on it
```{r}
...
preds
```
Annoyingly we need to extract the (only) element in `preds` (called 'fit') to 
get a table of positions and associated uncertainty.
```{r}
...
```
Let's plot these positions.  I'll use `ggplot2`, but you can uncomment the 
base R lines if you prefer those.
```{r}
library(ggplot2)
...
corpdfm2 <- dfm_trim(dfm(corp, remove = stopwords(), 
                         remove_punct = TRUE, 
                         stem = TRUE), 
                     min_termfreq = 10, min_docfreq = 3)
...
plot(ca_mod, what = c("all", "none")) # just speakers
```
