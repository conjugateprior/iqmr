...
table(ntoken(para_corp)) # a few less than 5 words
...
checkBeta(mod) # nothing should really load on just one topic
plot(mod)
plot(mod, type = "labels")

labelTopics(mod)
findThoughts(mod, texts = texts(para_corp), topics = 9)

mod <- stm(para_dfm, K = 10, 
           prevalence = ~ party, data = docvars(para_corp),
           seed = 12345)
gg <- estimateEffect(c(2,4,5,8) ~ party, mod, docvars(para_corp))
summary(gg)

plot(gg, "party", labeltype = "prob", model = mod)

summary(lm(I(mod$theta[,8]) ~ I(docvars(para_corp, "party"))))
