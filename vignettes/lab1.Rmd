...
To extract the 13th speaker's contribution
```{r}
corp[13]
```
Notice that this is a named character vector.  (`corp[13:14]` would also work).
...
corp[,'vote']
```
(`corp[['vote']]` also works, if you prefer to get a one column data frame).
...
words during it
```{r}
no_corp <- corpus_subset(corp, vote == "no" & ntoken(corp) > 100)
```
(Goodbye Mr Mahon)
...
sentences.
```{r}
para_corp <- corpus_reshape(corp, to = "paragraphs") # or "sentences"
