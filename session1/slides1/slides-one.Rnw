...
Rely on: instrumentality, randomness and reflexivity
...
Computer-assisted content analysis works best when language usage is \textit{institutionalized} 
\ita
\itm stable, conventionalized, and instrumental
\itz
Implicitly, we always condition on some institution, e.g.
\ita
\itm courts, legislatures, online political persuasion, sports or financial reporting, survey response
\itz
...
In constrast to
\ita
\itm conversation, technical manuals
\itz
%\slide{Modeling}
%
...
The standard set of equivalence assumptions are the `bag of words'.  Specifically:
...
\end{frame}
\begin{frame}[plain]
\centerline{\includegraphics[scale=.4]{pictures/topics2}}
\end{frame}

\begin{frame}[t]\frametitle{Classical content analysis}

Every word $W$ has an category $W$ 

The word to category mapping $\beta$ is provided by the researcher as a \textit{content analysis dictionary}

The content of a document $\theta$ is the proportion (or count) of each category

~\\
\centerline{\includegraphics[scale=.5]{pictures/new-topics-ca.png}}
