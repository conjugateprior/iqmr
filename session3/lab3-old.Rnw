...
Let's take another look at the US Senate debate on partial birth abortion. Yesterday we applied a dictionary-approach to analyze the vocabulary of the debates. Today, we want to use the speeches to estimate individual positions based on Senatorial rhetoric. 
...
Download the file `us-abortion-debate.csv' from the course website. This file is   a term-document matrix containing the word counts for each speaker during the debate. Open $R$. Let's first start by installing and loading the package \texttt{austin} that allows us to run \textit{Wordscores} or \textit{Wordfish}: 
<<load-library>>=
library(austin)
@
